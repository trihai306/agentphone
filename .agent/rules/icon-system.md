...
**BẮT BUỘC**: Mọi icons trong project PHẢI sử dụng hệ thống icons chuẩn từ sidebar.

## Icon Source

Icons được định nghĩa tập trung trong:
```
@/Components/Layout/NavLink.jsx → icons object
```

## Icon Style Standard

```jsx
// ✅ ĐÚNG: SVG stroke-based icon
...
### ✅ ĐÚNG: Sử dụng icon chuẩn

```jsx
// Định nghĩa icons object (copy từ NavLink.jsx hoặc import)
const icons = {
    home: "M3 12l2-2...",
    // ...
};
...
### ❌ KHÔNG dùng icon libraries bên ngoài
...
### ❌ KHÔNG dùng strokeWidth khác 1.5

```jsx
// ❌ WRONG: strokeWidth !== 1.5
...
Khi cần thêm icon mới:

1. **Tìm icon từ Heroicons Outline**: https://heroicons.com/ (chọn Outline style)
2. **Copy SVG path** (chỉ phần `d="..."`)
3. **Thêm vào `NavLink.jsx`** icons object
4. **Đặt tên theo convention**: camelCase, mô tả chức năng

```javascript
// NavLink.jsx
const icons = {
    // ... existing icons
    newIcon: "M... (path từ heroicons)",
};
```

---

## 🔧 AUTO-CREATE ICONS (MANDATORY)

**BẮT BUỘC**: Khi Agent cần sử dụng icon mà chưa có trong `NavLink.jsx`, Agent PHẢI:

### 1. Kiểm tra icon đã tồn tại chưa
```bash
# Tìm icon trong NavLink.jsx
grep -i "iconName:" laravel-backend/resources/js/Components/Layout/NavLink.jsx
```

### 2. Nếu chưa có → Tự động thêm vào NavLink.jsx

Thêm icon path mới vào object `icons` trong file `NavLink.jsx`:
...
### 3. Icon Path Sources (ưu tiên theo thứ tự)
...
```javascript
// Đây là các icons thường dùng, thêm vào NavLink.jsx nếu cần:
...
| Buttons | 16-20px | `w-4 h-4` hoặc `w-5 h-5` |
...
- [ ] Icon sử dụng `fill="none"` và `stroke="currentColor"`
- [ ] `strokeWidth={1.5}` (không phải 1 hoặc 2)
- [ ] `viewBox="0 0 24 24"`
- [ ] Không import từ thư viện bên ngoài
- [ ] Icon path được lấy từ `NavLink.jsx` hoặc Heroicons Outline