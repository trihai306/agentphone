...
/**
 * EditorToolbar - Top toolbar for the flow editor
 * Contains back button, flow name, device selector, execution controls, save/deploy buttons
 */
    ...
    const handleRunWorkflow = async () => {
        if (selectedDevice) {
            ...
            try {
                ...
                // Set all action nodes to 'pending' state
    ...
    return (
        <div className={`h-14 flex items-center justify-between px-4 flex-shrink-0 border-b transition-colors duration-300 ${isDark ? 'bg-[#0a0a0a] border-[#1e1e1e]' : 'bg-white border-gray-200 shadow-sm'}`}>
            ...
            <div className="flex items-center gap-1.5">
                ...
                <div className="relative device-selector-container">
                    ...
                    >
                        ...
                        {selectedDevice ? (
                            ...
                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${onlineDevices.length > 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-500'}`}>
                ...
                {isRunning && (
                    <>
                        <button onClick={pauseExecution} className="h-9 px-4 bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 border border-amber-500/30">
                        ...
                        <button onClick={stopExecution} className="h-9 px-4 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 border border-red-500/30">
                ...
                {isPaused && (
                    <>
                        <button onClick={resumeExecution} className="h-9 px-4 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 border border-emerald-500/30">
                ...
                {(isCompleted || hasError) && (
                    <button
                        ...
                        className={`h-9 px-4 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 border ${isCompleted ? 'bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border-emerald-500/30' : 'bg-red-500/20 hover:bg-red-500/30 text-red-400 border-red-500/30'}`}
                    >
                ...
                <button
                    ...
                    className={`h-8 px-2.5 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 border ${nodes.filter(n => n.data?.screenshotUrl).length > 0
                        ? isDark ? 'bg-purple-500/10 border-purple-500/30 text-purple-400 hover:bg-purple-500/20' : 'bg-purple-50 border-purple-200 text-purple-600 hover:bg-purple-100'
                        : isDark ? 'bg-[#1a1a1a] border-[#2a2a2a] text-gray-600 cursor-not-allowed' : 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed'
                        }`}
                ...
                <button
                    ...
                    className={`h-8 px-3 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 border ${saving
                        ...
                        : lastSaved
                            ...
                            : isDark ? 'bg-[#1a1a1a] hover:bg-[#252525] text-gray-300 border-[#2a2a2a]' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200'
                        }`}
                ...
                <div className="relative">
                    <button
                        ...
                        className={`h-8 px-2 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 border ${isDark ? 'bg-[#1a1a1a] hover:bg-[#252525] text-gray-300 border-[#2a2a2a] hover:border-[#3a3a3a]' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200 hover:border-gray-300'}`}
                    ...
                    {modals.langDropdown.isOpen && (
                        <div className={`absolute top-full right-0 mt-1 w-32 rounded-lg shadow-xl border overflow-hidden z-50 ${isDark ? 'bg-[#1a1a1a] border-[#2a2a2a]' : 'bg-white border-gray-200'}`}>
                            <button
                                onClick={() => { changeLanguage('vi'); closeModal('langDropdown'); }}
                                className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 transition-colors ${getCurrentLanguage() === 'vi' ? isDark ? 'bg-cyan-500/20 text-cyan-400' : 'bg-cyan-50 text-cyan-600' : isDark ? 'hover:bg-[#252525] text-gray-300' : 'hover:bg-gray-50 text-gray-700'}`}
                            >
                                <span>🇻🇳</span><span>Tiếng Việt</span>
                            </button>
                            <button
                                onClick={() => { changeLanguage('en'); closeModal('langDropdown'); }}
                                className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 transition-colors ${getCurrentLanguage() === 'en' ? isDark ? 'bg-cyan-500/20 text-cyan-400' : 'bg-cyan-50 text-cyan-600' : isDark ? 'hover:bg-[#252525] text-gray-300' : 'hover:bg-gray-50 text-gray-700'}`}
                            >
