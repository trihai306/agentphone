...
export default function NavLink({ href, icon, children, active = false, collapsed = false }) {
    ...
    if (collapsed) {
        return (
            <Link
                href={href}
                className={`relative flex items-center justify-center p-2.5 rounded-xl transition-all group ${
                    active
    ...
    return (
        <Link
            href={href}
            className={`flex items-center space-x-3 px-3 py-2 rounded-xl transition-all group ${
                active
                    ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-md'
