...
export default function Payment({ userPackage = {}, paymentMethods = [], bankInfo = {} }) {
    ...
    return (
        <AppLayout title={t('packages.payment', 'Payment')}>
            <Head title={t('packages.payment', 'Payment')} />
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div className="flex-1">
                            <h1 className={`text-2xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                                {t('packages.complete_payment', 'Complete Payment')}
                            </h1>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-1 lg:order-2">
                            <div className={`p-5 rounded-xl ${isDark ? 'bg-[#1a1a1a]' : 'bg-white border border-gray-200'}`}>
                                <h2 className={`text-sm font-medium mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{t('packages.order_summary', 'Order Summary')}</h2>

                                <div className="space-y-3 mb-4">
                                    <div className="flex justify-between text-sm">
                                        <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>{t('packages.package', 'Package')}</span>
                                        <span className={isDark ? 'text-white' : 'text-gray-900'}>{pkg.name}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>{t('packages.status', 'Status')}</span>
                                        <span className={`px-2 py-0.5 text-xs font-medium rounded ${isDark ? 'bg-amber-900/30 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>
                                            {t('packages.pending', 'Pending')}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className={`pt-4 border-t ${isDark ? 'border-[#2a2a2a]' : 'border-gray-100'}`}>
                                    <div className="flex justify-between">
                                        <span className={`text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('packages.total', 'Total')}</span>
                        ...
                        <div className="lg:col-span-2 lg:order-1 space-y-4">
                            {userPackage.payment_method === 'bank_transfer' && (
                                <div className={`p-5 rounded-xl ${isDark ? 'bg-[#1a1a1a]' : 'bg-white border border-gray-200'}`}>
                                    <h2 className={`text-sm font-medium mb-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{t('packages.bank_transfer_details', 'Bank Transfer Details')}</h2>

                                    <div className="space-y-4">
                                        <div className={`p-4 rounded-lg ${isDark ? 'bg-[#222]' : 'bg-gray-50'}`}>
                                            <div className="space-y-3">
                                                <div>
                                                    <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('packages.bank', 'Bank')}</span>
                                                    <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{bankInfo.bank_name}</p>
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('packages.account_number', 'Account Number')}</span>
                                                    ...
                                                    >
                                                        {copied === 'account' ? t('common.copied', 'Copied!') : t('common.copy', 'Copy')}
                                                    </button>
                                                </div>
                                                <div>
                                                    <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('packages.account_name', 'Account Name')}</span>
                                                    <p className={`font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{bankInfo.account_name}</p>
                                                </div>
                                                {bankInfo.branch && (
                                                    <div>
                                                        <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('packages.branch', 'Branch')}</span>
                                        ...
                                        <div className={`p-4 rounded-lg ${isDark ? 'bg-purple-900/20' : 'bg-purple-50'}`}>
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <span className={`text-xs ${isDark ? 'text-purple-400' : 'text-purple-600'}`}>{t('packages.transfer_description', 'Transfer Description (Required)')}</span>
                                                ...
                                                >
                                                    {copied === 'code' ? t('common.copied', 'Copied!') : t('common.copy', 'Copy')}
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className={`p-4 rounded-lg ${isDark ? 'bg-emerald-900/20' : 'bg-emerald-50'}`}>
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <span className={`text-xs ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>{t('packages.amount_to_transfer', 'Amount to Transfer')}</span>
                                                ...
                                                >
                                                    {copied === 'amount' ? t('common.copied', 'Copied!') : t('common.copy', 'Copy')}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {userPackage.payment_method !== 'bank_transfer' && (
                                <div className={`p-8 rounded-xl text-center ${isDark ? 'bg-[#1a1a1a]' : 'bg-white border border-gray-200'}`}>
                                    <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>
                                        {t('packages.redirecting', 'Redirecting to {{name}}...', { name: selectedMethod.name })}
                                    </p>
                                </div>
                            )}
                            ...
                            <div className={`p-4 rounded-lg ${isDark ? 'bg-blue-900/20' : 'bg-blue-50'}`}>
                                <h3 className={`text-sm font-medium mb-2 ${isDark ? 'text-blue-300' : 'text-blue-800'}`}>{t('packages.important_notes', 'Important Notes')}</h3>
                                <ul className={`text-xs space-y-1 ${isDark ? 'text-blue-300/80' : 'text-blue-700'}`}>
                                    <li>• {t('packages.note_description', 'Use exact transfer description for automatic verification')}</li>
                                    <li>• {t('packages.note_activation', 'Package activation within 5-15 minutes after confirmation')}</li>
                                    <li>• {t('packages.note_support', 'Contact support if you need assistance')}</li>
                                </ul>
                            </div>
                            ...
                            <div className="flex justify-end gap-3">
                                ...
                                >
                                    {t('packages.completed_payment', "I've Completed Payment")}
                                </button>
                            </div>
                        </div>
                    </div>
