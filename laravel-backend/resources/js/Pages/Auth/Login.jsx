    ...
    return (
        <AuthLayout>
            <div className={`p-8 rounded-2xl ${isDark ? 'bg-[#1a1a1a]' : 'bg-white border border-gray-200'}`}>
                ...
                {flash?.success && (
                    <div className={`mb-6 p-3 rounded-lg text-sm ${isDark ? 'bg-emerald-900/20 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}>
                        {flash.success}
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Link href="/forgot-password" className={`text-sm ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>
                    ...
                    >
                        {processing ? 'Signing in...' : t('auth.login.submit', { defaultValue: 'Sign In' })}
                    </button>
                </form>
                ...
                <p className={`mt-6 text-center text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                    ...
                    <Link href="/register" className={isDark ? 'text-white hover:underline' : 'text-gray-900 hover:underline'}>
                        {t('auth.login.register', { defaultValue: 'Sign up' })}
                    </Link>
                </p>
            </div>
        </AuthLayout>
