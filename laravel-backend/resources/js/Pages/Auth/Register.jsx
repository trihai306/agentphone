...
export default function Register() {
    ...
    return (<>
        <Head title={t('auth.register.head_title', { defaultValue: 'Register - CLICKAI' })} />
                ...
                <div className={`w-full max-w-[480px] transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    <div className="text-center mb-6">
                        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm mb-4 ${isDark ? 'bg-gradient-to-r from-emerald-500/10 to-teal-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-gradient-to-r from-emerald-500/10 to-teal-500/10 text-emerald-600 border border-emerald-500/20'}`}>
                            ...
                            {t('auth.register.badge', { defaultValue: 'Create new account' })}
                        </div>
                        <h1 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('auth.register.heading', { defaultValue: 'Start your journey' })}</h1>
                        <p className={`text-base ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{t('auth.register.subtitle', { defaultValue: 'Register free and discover the power of automation' })}</p>
                    </div>
                    ...
                    <div className={`relative rounded-2xl p-8 backdrop-blur-xl ${isDark ? 'bg-white/[0.03] border border-white/[0.08] shadow-2xl shadow-black/20' : 'bg-white/70 border border-white/50 shadow-2xl shadow-emerald-500/5'}`}>
                        ...
                        <form onSubmit={handleSubmit} className="relative space-y-4">
                            {/* Name */}
                            <div>
                                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('auth.register.name', { defaultValue: 'Full name' })}</label>
                                <div className="relative">
                                    ...
                                    <input type="text" value={data.name} onChange={e => setData('name', e.target.value)} className={inputClass(errors.name)} placeholder={t('auth.register.name_placeholder', { defaultValue: 'John Doe' })} autoComplete="name" autoFocus />
                                </div>
                                {errors.name && <p className="mt-2 text-xs text-red-500">{errors.name}</p>}
                            </div>
                            {/* Email */}
                            <div>
                                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('auth.register.email', { defaultValue: 'Email' })}</label>
                                <div className="relative">
                            ...
                            {/* Password */}
                            <div>
                                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('auth.register.password', { defaultValue: 'Password' })}</label>
                                <div className="relative">
                                ...
                                <p className={`mt-1.5 text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{t('auth.register.password_hint', { defaultValue: 'Minimum 8 characters' })}</p>
                            </div>
                            {/* Confirm */}
                            <div>
                                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('auth.register.password_confirmation', { defaultValue: 'Confirm password' })}</label>
                                <div className="relative">
                            ...
                            <div className="pt-2">
                                <label className="flex items-start gap-3 cursor-pointer">
                                    ...
                                    <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{t('auth.register.terms_agree', { defaultValue: 'I agree to the' })} <a href="/terms" className={isDark ? 'text-emerald-400 hover:underline' : 'text-emerald-600 hover:underline'}>{t('auth.register.terms', { defaultValue: 'Terms' })}</a> {t('common.and', { defaultValue: 'and' })} <a href="/privacy" className={isDark ? 'text-emerald-400 hover:underline' : 'text-emerald-600 hover:underline'}>{t('auth.register.privacy_policy', { defaultValue: 'Privacy Policy' })}</a></span>
                                </label>
                            ...
                            <button type="submit" disabled={processing} className={`relative w-full py-3.5 text-sm font-semibold rounded-xl transition-all duration-300 overflow-hidden group mt-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white hover:shadow-lg hover:shadow-emerald-500/30 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100`}>
                                ...
                                <span className="relative flex items-center justify-center gap-2">
                                    {processing ? <><svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{t('auth.register.processing', { defaultValue: 'Creating...' })}</>
                                        : <>{t('auth.register.submit', { defaultValue: 'Register' })}<svg className="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg></>}
                                </span>
                            </button>
                        </form>

                        {/* Divider & Social */}
                        <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className={`w-full border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`} /></div><div className="relative flex justify-center"><span className={`px-4 text-xs font-medium ${isDark ? 'bg-[#0d0d15] text-gray-500' : 'bg-white text-gray-400'}`}>{t('auth.register.or_register_with', { defaultValue: 'or register with' })}</span></div></div>
                    ...
                    <p className={`mt-8 text-center text-sm ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>{t('auth.register.have_account', { defaultValue: 'Already have an account?' })} <Link href="/login" className={`font-semibold ${isDark ? 'text-emerald-400 hover:text-emerald-300' : 'text-emerald-600 hover:text-emerald-700'}`}>{t('auth.register.login', { defaultValue: 'Login' })}</Link></p>
                    <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                        <FeatureBadge isDark={isDark} icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>} text={t('auth.register.free_badge', { defaultValue: '100% Free' })} />
                        <FeatureBadge isDark={isDark} icon={<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>} text={t('auth.register.security_badge', { defaultValue: 'High security' })} />
                    </div>
                </div>
            </main>
