...
export default function ForgotPassword() {
    ...
    return (<>
        <Head title={t('auth.forgot_password.head_title', { defaultValue: 'Forgot Password - CLICKAI' })} />
                    ...
                    <div className={`relative rounded-2xl p-8 backdrop-blur-xl ${isDark ? 'bg-white/[0.03] border border-white/[0.08] shadow-2xl shadow-black/20' : 'bg-white/70 border border-white/50 shadow-2xl shadow-amber-500/5'}`}>
                        ...
                        {!emailSent ? (<>
                            ...
                            <div className="relative text-center mb-8">
                                ...
                                <h1 className={`text-2xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('auth.forgot_password.title', { defaultValue: 'Forgot Password' })}</h1>
                                <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{t('auth.forgot_password.description', { defaultValue: 'Enter your email to receive a password reset link' })}</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="relative space-y-5">
                                <div>
                                    <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{t('auth.forgot_password.email', { defaultValue: 'Email' })}</label>
                                ...
                                <button type="submit" disabled={processing} className={`relative w-full py-3.5 text-sm font-semibold rounded-xl transition-all duration-300 overflow-hidden group bg-gradient-to-r from-amber-600 to-orange-600 text-white hover:shadow-lg hover:shadow-amber-500/30 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100`}>
                                    ...
                                    <span className="relative flex items-center justify-center gap-2">
                                        {processing ? <><svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{t('auth.forgot_password.processing', { defaultValue: 'Sending...' })}</> : <>{t('auth.forgot_password.submit', { defaultValue: 'Send link' })}<svg className="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></>}
                                    </span>
                                </button>
                            </form>
                            <div className="relative mt-6 text-center">
                                <Link href="/login" className={`inline-flex items-center gap-2 text-sm font-medium transition-colors ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>
                                    ...
                                    {t('auth.forgot_password.back_to_login', { defaultValue: 'Back to login' })}
                                </Link>
                            </div>
                        </>) : (<>
                            {/* Success State */}
                            <div className="relative text-center">
                                ...
                                <h2 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>{t('auth.forgot_password.check_email', { defaultValue: 'Check your email' })}</h2>
                                <p className={`text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{t('auth.forgot_password.email_sent', { defaultValue: 'We sent a password reset link to' })} <strong className={isDark ? 'text-white' : 'text-gray-900'}>{data.email}</strong></p>
                                <div className={`p-4 rounded-xl mb-6 text-left ${isDark ? 'bg-white/5 border border-white/10' : 'bg-gray-50 border border-gray-200'}`}>
                                    <p className={`text-xs font-medium mb-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{t('auth.forgot_password.no_email', { defaultValue: "Didn't receive the email?" })}</p>
                                    <ul className={`text-xs space-y-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                                        <li>• {t('auth.forgot_password.check_spam', { defaultValue: 'Check spam folder' })}</li>
                                        <li>• {t('auth.forgot_password.check_email_correct', { defaultValue: 'Make sure you entered the correct email' })}</li>
                                    </ul>
                                </div>
                                <div className="space-y-3">
                                    <button onClick={() => setEmailSent(false)} className={`w-full py-3 text-sm font-semibold rounded-xl transition-all duration-300 bg-gradient-to-r from-amber-600 to-orange-600 text-white hover:shadow-lg hover:shadow-amber-500/30`}>{t('auth.forgot_password.resend', { defaultValue: 'Resend email' })}</button>
                                    <Link href="/login" className={`block text-sm font-medium ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>{t('auth.forgot_password.back_to_login', { defaultValue: 'Back to login' })}</Link>
                                </div>
                            </div>
                        </>)}
                    </div>
