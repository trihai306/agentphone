...
class JobDispatchService
{
    ...
    /**
     * Generate action config using FlowService graph traversal
     * This ensures consistent behavior between test-runs and real jobs:
     * - Correct execution order (follows edges, not DB insertion order)
     * - Loop expansion
     * - Edge delay injection
    ...
    public function generateActionConfig(WorkflowJob $job, ?int $recordId = null): array
    {
        ...
        // Use FlowService for graph-aware traversal (same as test-run path)
        ...
        $flowContext = $variableContextService->buildContext($flow);
        $variables = array_merge($job->config['variables'] ?? [], $flowContext);
        ...
        Log::debug("Generated action config via graph traversal", [
            'job_id' => $job->id,
    ...
    // Note: mapNodeTypeToAction() removed - now using FlowService::mapNodeTypeToAction()
    // as the single source of truth for node type → action type mapping
}
